...
| find_leads (report) | outbound | `{"report_mode": true, "objective": str, "research_type": "lead_discovery" or "competitor_analysis" or "custom", "area": str, "limit": int}` | Agent plans the Places search and supporting website evidence, then synthesizes an objective-specific report into `tasks.result`. Places has a $0.50 event-charge cap and 180s timeout; optional website extraction is usage-billed and bounded to 60s / 6 pages / 3 sites. Does NOT write leads, draft emails, or trigger outreach |
...
| PUT /outbound/reports/schedule | dashboard | Updates schedule config `{objective, research_type, term, area, limit, cadence, time, timezone, weekday, enabled}`, calculates IANA next_run_at (Monday=0) |
| POST /outbound/reports/run | dashboard | Triggers manual report run immediately using saved schedule or optional body `{objective, research_type, term, area, limit}` (400 if no schedule, 409 if active run) |
...
The report worker runs in FastAPI lifespan on a single always-on instance, with a local `report_worker.lock` and 30-second idle poll interval. No distributed-host takeover is supported. Schedule inputs are owner `objective` (non-blank string up to 2000 chars), `research_type` (`lead_discovery` | `competitor_analysis` | `custom`), optional internal `term`, daily/weekly cadence, `HH:MM` local time, IANA timezone, Monday=0 weekday, enabled flag, area, and limit 1–50. Storage is `business_config.data.lead_report_schedule`; task creation uses an atomic reservation and task execution claims pending rows. Saving a schedule preserves in-flight reservation metadata and unrelated business configuration. Persistence failure returns 503 rather than acknowledging an unapplied schedule.

Reports use existing `tasks.kind=find_leads` with `payload.report_mode=true`, `tasks.result` for the report or failure, and `agent_events.ref` for the task's agent trace. No new migration or Manager session is required. A paused schedule can run manually; pausing never cancels an in-flight run. Running jobs interrupted by a service restart become failed, avoiding automatic repeated paid scraping. Apify actor execution is restricted to an allowlist (`compass/crawler-google-places`, `apify/website-content-crawler`) with SSRF public URL validation, non-blocking async DNS resolution, and event-charge capping ($0.50 max event-charge cap for Places scraper; supporting website crawler billed per Apify platform pay-per-usage, bounded to max 60s runtime and 6 total pages across max 3 candidate sites). Website crawler failures or timeouts are isolated and recorded as report limitations rather than failing the report run. Research generation uses a two-stage agent loop (`research_planner` for search query/evidence planning, `report_synthesizer` for report generation). Planner failure hard-fails the run. Competitor comparisons compare market evidence against full business config offerings and use 'unknown' when competitor pricing/offerings are missing. Cited evidence sources are strictly code-derived from actual retrieved evidence URLs.
